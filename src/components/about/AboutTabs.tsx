  ...
  return (
    ...
    >
      <Tabs defaultValue="vision" className="w-full h-full">
        <div className="backdrop-blur-xl bg-white/10 border border-white/20 rounded-lg md:rounded-xl p-2 md:p-6 shadow-lg h-[450px] flex flex-col relative overflow-hidden group">
          ...
          <TabsContent value="vision" className="mt-0 data-[state=active]:animate-fade-in w-full h-full relative z-10">
            <ScrollArea className={`${isSmallMobile ? 'h-[150px]' : (isMobile ? 'h-[200px]' : 'h-[300px]')} pr-2`}>
              <VisionTabContent />
            </ScrollArea>
          </TabsContent>
        </div>
