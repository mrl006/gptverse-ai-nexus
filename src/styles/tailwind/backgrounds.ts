
const backgrounds = {
  backgroundImage: {
    'hero-pattern': 'radial-gradient(circle at 50% 50%, rgba(14, 243, 75, 0.15) 0%, transparent 60%)',
    'main-gradient': 'linear-gradient(135deg, rgba(4, 8, 18, 0.95) 0%, rgba(6, 16, 26, 0.95) 100%)',
    'glass-gradient': 'linear-gradient(135deg, rgba(14, 243, 75, 0.05) 0%, rgba(0, 174, 255, 0.05) 100%)',
    'card-gradient': 'linear-gradient(135deg, rgba(6, 16, 26, 0.5) 0%, rgba(10, 24, 38, 0.2) 100%)',
    'neon-gradient': 'linear-gradient(90deg, #0ef34b 0%, #00aeff 100%)',
    'tech-grid': 'url(\'data:image/svg+xml;base64,\')'
  }
};

export default backgrounds;
